...
export class LinkTools {
    ...
    // Tax & Fee Calculation
    // ============================================================================
    calculateTaxAndFees(amount, taxRate = 0, feeRate = 0, taxInclusive = false) {
    ...
    async createLink(params) {
        try {
            ...
            const taxAndFees = this.calculateTaxAndFees(params.amount, params.tax_rate || 0, params.fee_rate || 0, params.tax_inclusive || false);
            const linkParams = {
                amount: params.amount,
            ...
            const link = await this.omiseClient.post('/links', linkParams);
            // Generate QR code
            ...
            this.logger.error('Failed to create payment link via MCP tool', error, params);
            return {
                success: false,
    ...
    async retrieveLink(params) {
        try {
            ...
            const link = await this.omiseClient.get(`/links/${params.link_id}`);
            // Generate QR code
            const qrCode = this.generateQRCode(link.payment_uri);
            // Calculate statistics
            const statistics = this.calculateLinkStatistics(link);
            ...
            this.logger.error('Failed to retrieve payment link via MCP tool', error, { linkId: params.link_id });
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error occurred'
            };
    ...
    async listLinks(params) {
        try {
            ...
            const links = await this.omiseClient.get('/links', queryParams);
            ...
            this.logger.error('Failed to list payment links via MCP tool', error, params);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error occurred'
            };
